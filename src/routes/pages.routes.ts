...
import { getUserCartService } from "../services/items.service.js";
...
const router = Router();
    ...
    router.get("/", optionalAuth, async (req: AuthRequest, res) => {
        ...
        const userCart = req.userId ? await getUserCartService(req.userId!) : { items: [], totalCount: 0 }; // Уявна функція, яка дістає товари в кошику користувача
    ...
    router.get("/profile", requireAuth, async (req: AuthRequest, res) => {
        ...
        const userCart = await getUserCartService(req.userId!);
        console.log("User Cart:", userCart); // Додаємо логування для перевірки
    ...
    router.get("/order", requireAuth, async (req: AuthRequest, res) => {
        const userData = await getFullUserData(req.userId!);
